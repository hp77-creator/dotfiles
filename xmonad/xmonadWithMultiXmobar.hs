...
myWindowGap = 5  :: Integer -- this determines distance between two windows
...
myManageHook = composeAll
    ...
    , className =? "Free Download Manager" --> doShift (myWorkspaces !! 4) -- fun
    , className =? "FreeTube" --> doShift (myWorkspaces !! 4) --fun
    , className =? "Franz" --> doShift (myWorkspaces !! 3) --chat
    , className =? "Wine" --> doShift (myWorkspaces !! 1) --code
...
main = do
 xmproc0 <- spawnPipe ("xmobar -x 0 $HOME/.config/xmobar/xmobarrc1") -- first xmobar
 xmproc1 <- spawnPipe ("xmobar -x 0 $HOME/.config/xmobar/xmobarrc2") -- second xmobar
 xmproc2 <- spawnPipe ("xmobar -x 0 $HOME/.config/xmobar/xmobarrc3") -- third xmobar
 xmonad $ docks  $ defaults {logHook            = dynamicLogWithPP  xmobarPP
                               { ppOutput          = \x ->  hPutStrLn xmproc0 x
                               , ppCurrent         = xmobarColor "#ddbd94" "" . wrap
                                                     ("<box type=Bottom width=2 mb=1 color=" ++ "#ddbd94" ++">") "</box>"  -- Current workspace in xmobar
                               , ppVisible         = xmobarColor "#ddbd94" ""                -- Visible but not current workspace
                               , ppHidden          = xmobarColor "#c15c2e" "" . wrap
                                                     ("<box type=Top width=2 mt=2 color=" ++ "#c15c2e" ++ ">") "</box>"-- Hidden workspaces in xmobar
                               , ppHiddenNoWindows = xmobarColor "#c15c2e" ""                 -- Hidden workspaces (no windows)
